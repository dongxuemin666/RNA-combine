...
#sort
$samtools_path/samtools sort -o ${pre_name}.sorted.bam $bam_path/${pre_name}.bam
#mark dupicates
time $gatk_path/gatk MarkDuplicates -I ${pre_name}.sorted.bam -O ${pre_name}.markdup.bam -M ${pre_name}.metrics --CREATE_INDEX
...
time $gatk_path/gatk HaplotypeCaller -R $genome -I ${pre_name}.markdup.bam -ERC GVCF  -O ${pre_name}.snps.indels.vcf

fi

if [ "$sample_amount" == "single" ]; then

time $gatk_path/gatk HaplotypeCaller -R $genome -I ${pre_name}.markdup.bam -O ${pre_name}.snps.indels.vcf
...
#merge
time $gatk_path/gatk  GenomicsDBImport --genomicsdb-workspace-path my_database -L $interval \
--sample-name-map vcf_map.txt

time $gatk_path/gatk GenotypeGVCFs -R $genome -V gendb://my_database -O genotype.vcf.gz
...
# select SNPs
time $gatk_path/gatk SelectVariants -select-type SNP -V genotype.vcf.gz -O genotype.snp.vcf.gz

#filter SNPs
time $gatk_path/gatk VariantFiltration -V genotype.snp.vcf.gz \
    --filter-expression "QD < 2.0 || MQ < 40.0 || FS > 60.0 || SOR > 3.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0" \
    --filter-name "Filter" \
    -O genotype.snp.filter.vcf.gz
# select Indels
time $gatk_path/gatk SelectVariants -select-type INDEL \
    -V genotype.vcf.gz \
    -O genotype.indel.vcf.gz
#fileter Indels
time $gatk_path/gatk VariantFiltration -V genotype.indel.vcf.gz \
...
time $gatk_path/gatk MergeVcfs \
    -I genotype.snp.filter.vcf.gz -I genotype.indel.filter.vcf.gz \
    -O genotype.filter.snp.indel.vcf.gz
    
$gatk_path/gatk SelectVariants -R $genome -V genotype.filter.snp.indel.vcf.gz \
